			     ...
			     char __user *optval, int __user *optlen)
{
	...
	case PACKET_ROLLOVER_STATS:
		...
		if (rollover) {
			rstats.tp_all = atomic_long_read_unchecked(&rollover->num);
			rstats.tp_huge = atomic_long_read_unchecked(&rollover->num_huge);
			rstats.tp_failed = atomic_long_read_unchecked(&rollover->num_failed);
			data = &rstats;
			lv = sizeof(rstats);
		}
		rcu_read_unlock();
